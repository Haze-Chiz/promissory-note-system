...
def log_action(user_name, action):
    ...
    log = SystemLog(user_name=user_name, action=action, timestamp=datetime.utcnow())
    db.session.add(log)
    db.session.commit()
...
def dashboard():
    ...
    active_course = getattr(active_settings, 'active_course', 'Not Set (using list model)')
...
def accounts():
    ...
    pagination = query.order_by(Account.id.desc()).paginate(page=page, per_page=per_page, error_out=False)
...
def add_new_account():
    if request.method == "POST":
        ...
        log_action(session.get("user_name", "Admin User"), f"Added new account: {account.full_name} ({email})")
...
def edit_account(account_id):
    ...
    if request.method == "POST":
        if "reset_password" in request.form:
            ...
            log_action(session.get("user_name", "Admin User"), f"Reset password for {account.full_name}")
        else:
            ...
            log_action(session.get("user_name", "Admin User"), f"Updated account: {account.full_name}")
...
@admin_bp.route("/delete_account/<int:account_id>", methods=["POST"])
@require_role("Admin")
def delete_account(account_id):
    ...
    log_action(session.get("user_name", "Admin User"), f"Deleted account: {account.full_name}")
...
def logs():
    ...
    logs_pagination = query.order_by(SystemLog.timestamp.desc()).paginate(page=page, per_page=per_page, error_out=False)
...
# ------------------- Other routes -------------------
# school_year, semester, course, export, upload, logout etc.
# Add log_action calls where necessary in other routes
# ------------------- Semester, School Year, Course -------------------
...
def semester():
    ...
    if request.method == "POST":
        ...
        active_settings.active_semester = request.form.get("semester", "").strip()
        db.session.commit()
        flash("Active semester updated successfully!", "success")
        log_action(session.get("user_name", "Admin User"), f"Changed semester from '{old_semester}' to '{active_settings.active_semester}'")
...
def school_year():
    ...
    if request.method == "POST":
        ...
        active_settings.active_school_year = request.form.get("school_year", "").strip()
        db.session.commit()
        flash("Active school year updated successfully!", "success")
        log_action(session.get("user_name", "Admin User"), f"Changed school year from '{old_year}' to '{active_settings.active_school_year}'")
...
def course():
    if request.method == "POST":
        ...
        log_action(session.get("user_name", "Admin User"), f"Added new course '{course_name}'")
        return redirect(url_for("admin.course"))
...
def delete_course(course_id):
    ...
    log_action(session.get("user_name", "Admin User"), f"Deleted course '{course.name}'")
    return redirect(url_for("admin.course"))
...
def upload_accounts():
    ...
    try:
        ...
        if uploaded_rows:
            ...
            log_action(session.get("user_name", "Admin User"), f"Uploaded accounts: {', '.join(uploaded_rows)}")
        else:
...
def download_template():
    ...
    out.seek(0)
    log_action(session.get("user_name", "Admin User"), "Downloaded account upload template")
    return send_file(
