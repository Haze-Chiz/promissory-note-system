...
import random
import string
from models import db, Account, ActiveSettings, ActiveCourse
...
@admin_bp.route("/generated_password")
@require_role("Admin")
def show_generated_password():
    pwd = request.args.get("pwd")
    email = request.args.get("email")
    return render_template("admin/generated_password.html", pwd=pwd, email=email)
...
def upload_accounts():
    ...
    try:
        ...
        if uploaded_rows:
            ...
            flash("No new accounts were added (all emails already exist or invalid).", "info")
...
def download_template():
    headers = ["first_name", "middle_name", "last_name", "suffix", "email", "role", "status", "year_level", "course"]
...
@require_role("Admin")
def settings():
    return render_template("admin/settings.html", data={"admin_user": session.get("user_name", "Admin User")})
